# Deploy LINE Bot Ruby Examples on Render

This repo can be used to deploy ruby examples in the [line-bot-sdk-ruby](https://github.com/line/line-bot-sdk-ruby/tree/master/examples) on [Render](https://render.com/).
## Prerequisites
...
5. Once the bot servcie is live, find the service `onrender` URL (e.g., `https://line-bot-ruby-<something unique>.onrender.com`) on the Dashboard. Append `/callback` to the service URL to build the webhook URL (e.g., `https://line-bot-ruby-<something unique>.onrender.com/callback`). Paste the webhook URL to the `Webhook settings` section on the LINE channel's `Messaging API` tab on the [LINE Developers Console](https://developers.line.biz/console/). Also enable `Use webhook` on the same section.
...
- In the `render.yaml`, `BUNDLE_DEPLOYMENT=false` is not needed if there is `gemfile.lock` in your repo.
